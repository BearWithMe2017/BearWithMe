...
Shader "Shader Forge/AttemptNo" {
    Properties {
        ...
        _Color ("Color", Color) = (0,0.2425578,1,1)
...
/////// Diffuse:
                ...
                float node_537 = 1.0;
                float3 w = float3(node_537,node_537,node_537)*0.5; // Light wrapping
                float3 NdotLWrap = NdotL * ( 1.0 - w );
                float3 forwardLight = max(float3(0.0,0.0,0.0), NdotLWrap + w );
                float3 backLight = max(float3(0.0,0.0,0.0), -NdotLWrap + w ) * float3(node_537,node_537,node_537);
...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float specularMonochrome;
                float3 specColor;
                diffColor = DiffuseAndSpecularFromMetallic( diffColor, _Value, specColor, specularMonochrome );
