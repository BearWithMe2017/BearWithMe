    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float4 node_3260 = _Time;
                float2 node_8158 = (i.uv0+node_3260.g*float2(0.02,0.02));
...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float2 node_8158 = (i.uv0+node_8753.g*float2(0.02,0.02));
                float3 _Refraction_var = UnpackNormal(tex2D(_Refraction,TRANSFORM_TEX(node_8158, _Refraction)));
                float3 normalLocal = normalize(lerp(float3(0,0,1),_Refraction_var.rgb,_Reflection));
                float3 normalDirection = normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
                float2 sceneUVs = (i.projPos.xy / i.projPos.w) + (_Refraction_var.rgb.rg*(_Distortion*0.2));
